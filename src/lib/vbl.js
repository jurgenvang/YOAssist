...
export function extraheerTeams(json, clubGuid) {
  ...
  for (const obj of alleObjecten(json)) {
    for (const [sleutel, waarde] of Object.entries(obj)) {
      ...
      // De API geeft ook een leesbare categorie mee ('Heren Senioren'). Die
      // bewaren we voor de weergave; de code uit de GUID blijft leidend.
      const categorie = typeof obj.categorie === 'string' ? obj.categorie.trim() : null;
      if (!gevonden.has(guid)) gevonden.set(guid, { guid, naam, categorie });
    }
  }
...
/**
 * Link naar het wedstrijdblad op de publieke site van Basketbal Vlaanderen.
 * 'BVBL26279100BLAJ18PJBC' -> https://vblweb.wisseq.eu/Home/MatchDetail?wedguid=...
 *
 * Bewust afgeleid en niet opgeslagen: de GUID hebben we al, en een bewaarde URL
 * is een tweede waarheid die kan verouderen als Wisseq zijn paden wijzigt.
 */
export function wedstrijdbladUrl(guid) {
  if (typeof guid !== 'string' || !guid.trim()) return null;
  return `https://vblweb.wisseq.eu/Home/MatchDetail?wedguid=${encodeURIComponent(guid.trim())}`;
}

/**
 * De drieletterige categoriecode uit een ploeg-GUID.
 * 'BVBL1125HSE  2' met club 'BVBL1125' -> 'HSE'
 *
 * De code staat direct achter de club-GUID. Betrouwbaarder dan de teamnaam,
 * die vrije tekst is en per club anders geschreven wordt.
 */
export function categorieUitTeamGuid(teamGuid, clubGuid) {
  if (typeof teamGuid !== 'string' || typeof clubGuid !== 'string') return null;
  if (!teamGuid.startsWith(clubGuid)) return null;
  const code = teamGuid.slice(clubGuid.length, clubGuid.length + 3).trim().toUpperCase();
  return /^[A-Z0-9]{3}$/.test(code) ? code : null;
}
...
export function normaliseerWedstrijd(rauw) {
  ...
  // wedOff is een array van namen. Leeg of ontbrekend betekent: nog niemand
  // aangeduid. Eén naam betekent: maar één scheidsrechter — voor de
  // woensdagregel telt dat even hard als geen.
  const offNamen = Array.isArray(rauw.wedOff)
    ? rauw.wedOff.map((n) => String(n).trim()).filter(Boolean)
    : typeof rauw.wedOff === 'string' && rauw.wedOff.trim()
      ? [rauw.wedOff.trim()]
      : [];
  ...
  return {
    ...
    accGuid: (rauw.accGUID ?? '').trim() || null,
    offNamen,
    offAantal: offNamen.length,
